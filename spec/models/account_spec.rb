...
RSpec.describe Account, type: :model do
  ...
  describe "when creating a new account via webfinger" do
    ...
    let(:george_ap_server_webfinger_info) { {"subject" => "acct:george@activitypub.test", "links"=>[{"rel"=>"self", "type"=>"application/activity+json", "href"=>"https://activitypub.test/users/george" }]} }
 ...
 "publicKey"=>
   ...
   "publicKeyPem"=>
    ...
    let(:george_actor_info) { {"id"=>"https://activitypub.test/users/george",
 "preferredUsername"=>"george",
 "name"=>"George",
 "@context"=>"https://www.w3.org/ns/activitystreams",
 "publicKey"=>
    {"id"=>"https://activitypub.test/users/george#main-key",
     "owner"=>"https://activitypub.test/users/george",
   "publicKeyPem"=>
    "-----BEGIN PUBLIC KEY-----\\nVQIDAQAB\n-----END PUBLIC KEY-----\n"}} }
    ...
    before do
      ...
      allow(WebFinger).to receive(:discover!).with('acct:george@example.com').and_return(george_webfinger_info)
      ...
      allow(WebFinger).to receive(:discover!).with('acct:george@activitypub.test').and_return(george_ap_server_webfinger_info)
      ...
      allow(Account).to receive(:fetch_actor).with('https://activitypub.test/users/george').and_return(george_actor_info)
    end
    ...
    it "should not use the custom domain if subject returned by activiypub server is different than original subject" do
      account = Account.fetch_and_create_mastodon_account_by_address('george@example.com')
      expect(account.domain).to eq('activitypub.test')
    end
